# Terraform Enterprise Podman Compose Setup on AWS RHEL9 using Podman on privileged ports with self-signed certs and external S3

This setup deploys Terraform Enterprise v1.1.2 with Podman Compose that includes PostgreSQL and Redis on EC2 using self-signed certificates. This example uses privileged ports because CLI remote runs connect back on 443 leading to connection timed out errors on non-privileged ports. Users can pass in environment variables to configure external S3 storage. This allows validation of various S3-compatible object storage options with TFE.

> [!WARNING]
> This setup is intended for **development, testing, and validation purposes only**. It uses self-signed certificates, default passwords, runs TFE on privileged ports and as the `root` user, and is not hardened for production environments.
