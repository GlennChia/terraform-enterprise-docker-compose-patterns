# Terraform Enterprise Podman Compose Setup on AWS RHEL9 using Podman with self-signed certs and external S3

This setup deploys Terraform Enterprise v1.1.2 with Podman Compose that includes PostgreSQL and Redis on EC2 using self-signed certificates. Users can pass in environment variables to configure external S3 storage. This allows validation of various S3-compatible object storage options with TFE.
...
- RHEL9 EC2 instance created in a public subnet. EC2 installed with Podman Compose. Podman Compose includes TFE, PostgreSQL, and Redis containers
- External S3 is referenced in Podman Compose via environment variables, allowing various S3 object stores to be validated with TFE
...
Create a `~/.terraformrc` file with the following content, replacing `xxxxxx.atlasv1.zzzzzzzzzzzzz` with the team token created in the previous step. Replace the hostname `tfe.ec2` if it was changed.
...
Once the resource has been created, the CLI shows `Releasing state lock. This may take a few moments...`

![terraform apply releasing state lock](./docs/03-testing/04-terraform-commands/03-terraform-apply-releasing-state-lock.png)

Terraform apply complete

![terraform apply complete](./docs/03-testing/04-terraform-commands/04-terraform-apply-complete.png)

TFE workspace states

![tfe workspace states](./docs/03-testing/04-terraform-commands/05-tfe-workspace-states.png)

State details

![tfe workspace state details](./docs/03-testing/04-terraform-commands/06-tfe-workspace-state-details.png)

The `archivistterraform` directory now contains the `states` sub-directory

![s3 archivistterraform](./docs/03-testing/04-terraform-commands/07-s3-archivistterraform.png)

The `states` directory contains the state directory with an ID that matches the state ID in TFE.

![s3 archivistterraform states](./docs/03-testing/04-terraform-commands/08-s3-archivistterraform-states.png)

This contains 2 other directories

![s3 archivistterraform state sub dir](./docs/03-testing/04-terraform-commands/09-s3-archivistterraform-state-sub-dir.png)

View file in directory 1

![s3 archivistterraform state file1](./docs/03-testing/04-terraform-commands/10-s3-archivistterraform-state-file1.png)

View file in directory 2

![s3 archivistterraform state file2](./docs/03-testing/04-terraform-commands/11-s3-archivistterraform-state-file2.png)
